...
object PrefixSuffixTestDiscoveringSuite {
  ...
  private val testDiscoveryArgs = {
    val testDiscoveryArgsPath = System.getProperty("bazel.discover.classes.args.path")
  ...
  private def archivesPath: String =
    testDiscoveryArgs.getProperty("bazel.discover.classes.archives.file.paths") //this is set by scala_junit_test rule in scala.bzl

  private def suffixesWithClassSuffix: Set[String] =
    suffixes.map(_ + ".class")

  private def suffixes: Set[String] =
    parseProperty(testDiscoveryArgs.getProperty("bazel.discover.classes.suffixes"))

  private def prefixes: Set[String] =
    parseProperty(testDiscoveryArgs.getProperty("bazel.discover.classes.prefixes"))
  ...
  private def printDiscoveredClasses: Boolean =
    testDiscoveryArgs.getProperty("bazel.discover.classes.print.discovered").toBoolean
