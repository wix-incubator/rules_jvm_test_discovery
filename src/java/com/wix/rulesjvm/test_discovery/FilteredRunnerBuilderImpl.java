...
class FilteredRunnerBuilderImpl extends RunnerBuilder {

  private RunnerBuilder builder;
  private FilteredRunnerBuilder filteringRunnerBuilder;

  public FilteredRunnerBuilderImpl(RunnerBuilder builder, FilteredRunnerBuilder filteringRunnerBuilder) {
    this.builder = builder;
    this.filteringRunnerBuilder = filteringRunnerBuilder;
  }

  // Defined by --test_filter bazel flag.
  private Optional<Pattern> maybePattern = Optional.ofNullable(System.getenv("TESTBRIDGE_TEST_ONLY")).map(e -> Pattern.compile(e));

  @Override
  public Runner runnerForClass(Class<?> testClass) throws Throwable {
    Runner runner = builder.runnerForClass(testClass);

    return maybePattern.map(p -> filteringRunnerBuilder.f(runner, testClass, p)).orElse(runner);
  }
}
...
class JUnitFilteringRunnerBuilder implements FilteredRunnerBuilder {
  private final String TestClassFieldPreJUnit4_12 = "fTestClass";
  private final String TestClassField = "testClass";

  @Override
  public Runner f(Runner runner, Class<?> testClass, Pattern pattern) {
